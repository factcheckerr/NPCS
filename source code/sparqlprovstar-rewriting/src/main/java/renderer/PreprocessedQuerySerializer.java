...
import renderer.SerializableParsedTupleQuery.QueryModifier;
...
public class PreprocessedQuerySerializer extends AbstractQueryModelVisitor<RuntimeException> {
	...
	private boolean tripleRefExisit=false;
	...
	public void meet(MultiProjection node) throws RuntimeException {
		...
		for (ProjectionElemList proj : node.getProjections()) {
			for (ProjectionElem elem : proj.getElements()) {
				if (valueMap.containsKey(elem.getTargetName())) {
					ValueExpr expr = valueMap.get(elem.getTargetName());
					if (expr instanceof BNodeGenerator) {
						builder.append("_:" + elem.getTargetName());
					} else {
						valueMap.get(elem.getTargetName()).visit(this);
					}
				} else {
					builder.append("?" + elem.getTargetName());
				}
				builder.append(" ");
	...
	public void meet(ProjectionElem node) throws RuntimeException {
		if (node.getSourceExpression() == null) {
			...
			if (node.getTargetName().isEmpty() || node.getTargetName().equals(node.getSourceName())) {
				if (isDescribe && this.currentQueryProfile.extensionElements.containsKey(node.getSourceName())) {
					ExtensionElem elem = this.currentQueryProfile.extensionElements.get(node.getSourceName());

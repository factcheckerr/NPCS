package npcs.rewriting.wikidata;
	...
	public QueryModelNode Reify(QueryModelNode node,Var sprovnum)
	{
		...
		*/
		...
		// QueryModelNode currentnode = node.clone();
		 ...
		 //Var subjectprefix= rightJoin.getPredicateVar();
		
		// String updatedUrl = (rightJoin.getPredicateVar().getValue().toString()).replaceFirst("^http://", "");
        
		 
		// ValueConstant subjectprefixurl=new ValueConstant(vf.createIRI("http://example.org/p/"+updatedUrl));
	//	 ValueConstant predictaeprefixurl=new ValueConstant(vf.createIRI("http://example.org/ps/"+updatedUrl));
		
	
		// StatementPattern subjectSP=new StatementPattern();
		// StatementPattern predicateSP=new StatementPattern();
	
	//	 subjectSP.setSubjectVar(rightJoin.getSubjectVar());
	//	 subjectSP.setPredicateVar(createConstVar(subjectprefixurl.getValue()));
		/* subjectSP.setObjectVar(sprovnum);
	
	       subjectSP.setScope(rightJoin.getScope());
	       
	       predicateSP.setSubjectVar(sprovnum);
	
	       predicateSP.setPredicateVar(createConstVar(predictaeprefixurl.getValue()));
	       predicateSP.setObjectVar(rightJoin.getObjectVar());
...
*/
	    ...
	    //wikidata
	       QueryModelNode currentnode = node.clone();
			 ...
			 StatementPattern predicateSP=new StatementPattern();
			 subjectSP.setSubjectVar(rightJoin.getSubjectVar());
			////System.out.println(rightJoin.getPredicateVar().toString());
			// String updatedUrl="";
			 String updatedUrl = (rightJoin.getPredicateVar().getValue().toString()).replaceFirst("^http://", "");
		        
			 if((rightJoin.getPredicateVar().getName().startsWith("_const")))
				{
				 ValueConstant subjectprefixurl=new ValueConstant(vf.createIRI("http://example.org/p/"+updatedUrl));
				...
				//updatedUrl = (rightJoin.getPredicateVar().getValue().toString()).replaceFirst("^http://www.wikidata.org/prop/direct/", "");	
				// ValueConstant subjectprefixurl=new ValueConstant(vf.createIRI("http://www.wikidata.org/prop/"+updatedUrl));
				...
				{
					subjectSP.setPredicateVar(rightJoin.getPredicateVar());
				}
		       ...
		       if((rightJoin.getPredicateVar().getName().startsWith("_const")))
				{
		    	   ValueConstant predictaeprefixurl=new ValueConstant(vf.createIRI("http://example.org/ps/"+updatedUrl));
		    	   predicateSP.setPredicateVar(createConstVar(predictaeprefixurl.getValue()));
		    	   //   //System.out.println(rightJoin.getPredicateVar().getName());
		    	 //  ValueConstant predictaeprefixurl=new ValueConstant(vf.createIRI("http://www.wikidata.org/prop/statement/"+updatedUrl));
		       //predicateSP.setPredicateVar(createConstVar(predictaeprefixurl.getValue()));
				}	else
				{
					predicateSP.setPredicateVar(new Var(rightJoin.getPredicateVar().getName()+"xyz"));

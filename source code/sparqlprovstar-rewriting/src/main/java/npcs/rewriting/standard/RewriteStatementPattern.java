...
*/
	...
	public QueryModelNode Reify(QueryModelNode node,Var sprovnum)
	{
		...
		//standard
		 QueryModelNode currentnode = node.clone();
		 TupleExpr tupleExpr = (TupleExpr)currentnode;
		 QueryModelNode currentnode1 = node.clone();
		 TupleExpr tupleExpr1 = (TupleExpr)currentnode1;
		 StatementPattern rightJoin = (StatementPattern) tupleExpr1;
		// rightJoin.setContextVar(sprovnum);
		// rightJoin.setScope(Scope.NAMED_CONTEXTS);
		//  //System.out.println("rightJoin"+rightJoin);
		// StatementPattern subjectSP=(StatementPattern) tupleExpr1;
		// StatementPattern predicateSP=(StatementPattern) tupleExpr1;
		// StatementPattern objectSP=(StatementPattern) tupleExpr1;
		 StatementPattern subjectSP=new StatementPattern();
		 StatementPattern predicateSP=new StatementPattern();
		 StatementPattern objectSP=new StatementPattern();
		 subjectSP.setSubjectVar(sprovnum);
		 ValueConstant subjectprefix=new ValueConstant(vf.createIRI("http://www.w3.org/1999/02/22-rdf-syntax-ns#subject"));
	       subjectSP.setPredicateVar(createConstVar(subjectprefix.getValue()));
	       subjectSP.setObjectVar(rightJoin.getSubjectVar());
	   //  //System.out.println("Suject"+subjectSP);  
	       subjectSP.setScope(rightJoin.getScope());
	       predicateSP.setSubjectVar(sprovnum);
		 ValueConstant predicateprefix=new ValueConstant(vf.createIRI("http://www.w3.org/1999/02/22-rdf-syntax-ns#predicate"));
	       predicateSP.setPredicateVar(createConstVar(predicateprefix.getValue()));
	       predicateSP.setObjectVar(rightJoin.getPredicateVar());
	      // //System.out.println("predicateSP"+predicateSP);  
	       predicateSP.setScope(rightJoin.getScope());
	       objectSP.setSubjectVar(sprovnum);
	       ValueConstant objectprefix=new ValueConstant(vf.createIRI("http://www.w3.org/1999/02/22-rdf-syntax-ns#object"));
	       objectSP.setPredicateVar(createConstVar(objectprefix.getValue()));
	       objectSP.setObjectVar(rightJoin.getObjectVar());
	      // //System.out.println("objectSP"+objectSP); 
	       objectSP.setScope(rightJoin.getScope());
	       Join subpred = new Join();    				
	       subpred.setLeftArg(subjectSP);
	       subpred.setRightArg(predicateSP);
	     //  //System.out.println("new Join"+subpred);
	       Join newjoin = new Join();    				
	       	newjoin.setLeftArg(subpred);
	       	newjoin.setRightArg(objectSP);
		return newjoin;
	...
	public QueryModelNode handlemultiplejoinBindAndGroup(QueryModelNode node, List<Var> totaljoinstatementsprovenanceVariables, List<Var> variablesstart, Var finalprovennacevariable,int number) throws VisitorException {
		...
		this.join_number=number+1;
		 ...
		 else if(node instanceof LeftJoin)
		 {
			 ...
			 //System.out.println("gghhhhhhhhjjjjjjjjjjjjjjjjjjjjjjjjjabcdefg");	
		//System.out.println(totaljoinstatementsprovenanceVariables);	 
		//System.out.println(totaljoinstatementsprovenanceVariables);	 	 
				//
		if (!totaljoinstatementsprovenanceVariables.isEmpty()) {
